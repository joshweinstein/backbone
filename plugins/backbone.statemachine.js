...
Backbone.StatefulView = (function(Backbone, _){
    ...
    _.extend(StatefulView.prototype, Backbone.View.prototype, Backbone.StateMachine, {
        ...
        toState: function(name) {
            if (this.el) {
                $j(this.el).removeClass((this.stateClassName || ''));
                this.stateClassName = (this._states[name].className || name);
                $j(this.el).addClass(this.stateClassName);
            }
    ...
    Backbone.StateMachine.DebugView = Backbone.View.extend({
        ...
        render: function() {
            ...
            if (!this.rendered) {
                ...
                var innerHtml = $j('<div class="state"></div><a class="log">console.log</a>');
                $j(this.el).append(innerHtml);
                ...
                if (this.model.hasOwnProperty('el')) {
                    $j(this.el).hover(_.bind(function(){
                        var modelEl = $j(this.model.el);
                        this.cssMem = {'background-color': '', 'border': ''};
                        modelEl.css({'background-color': 'blue','border': '3px solid DarkBlue'});
                    }, this));
                    $j(this.el).mouseleave(_.bind(function(){
                        var modelEl = $j(this.model.el);
        ...
        register: function(instance) {
            ...
            if (this.viewsArray.length === 0) {
                var container = this.el = $j('<div id="backbone-statemachine-debug-container">'+
                ...
                $j('#backbone-statemachine-debug-hideshow', this.el).click(function(event){
                    event.preventDefault();
                    if (this.collapsed) {
                        $j(container).removeClass('collapsed').children().show();
                        $j(this).html('hide');
                        this.collapsed = false;
                    } else {
                        $j(container).addClass('collapsed').children().hide();
                        $j(this).html('show').show();
                        this.collapsed = true;
                    }
                });
            }
            ...
            $j(debugView.el).appendTo(this.el).css({'background-color': bgColor});
            debugView.render();
            if (this.collapsed) {
                $j(debugView.el).hide();
            }
